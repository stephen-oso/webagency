...
export default function Page({ data }) {
  ...
  return (
    <>
      <Head>
        <title>{business_data.name || 'Trades & Contracting'}</title>
      ...
      <section className="hero" style={hero ? { backgroundImage: `url(${hero})` } : {}}>
        ...
        <div className="hero-body">
          <p className="eyebrow">{business_data.city}, {business_data.state} · Licensed & Insured</p>
          <h1>{copy.headline}</h1>
          <p className="hero-sub">{copy.subheadline}</p>
          <div className="hero-actions">
            <a href="#contact" className="btn-yellow">{copy.cta_text || 'Request a Quote'}</a>
            {business_data.phone && (
          ...
          <div className="about-stats">
            {business_data.rating && (
              <div className="about-stat">
                <span className="big-num">★{business_data.rating}</span>
                <span>{business_data.review_count?.toLocaleString()} reviews</span>
              </div>
            )}
            <div className="about-stat">
              <span className="big-num">{business_data.city}</span>
              <span>{business_data.state}</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="info" id="contact">
        <div className="wrap">
          ...
          <a href="#contact" className="quote-cta">{copy.cta_text || 'Request a Free Quote'}</a>
        </div>
      </section>
