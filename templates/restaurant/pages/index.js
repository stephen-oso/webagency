...
export default function Page({ data }) {
  ...
  return (
    <>
      <Head>
        <title>{business_data.name || 'Restaurant'}</title>
      ...
      <nav className="nav">
        <div className="nav-inner">
          ...
          <a href="#contact" className="nav-cta">{copy.cta_text || 'Reserve a Table'}</a>
        </div>
      </nav>
      ...
      <section className="hero" style={hero ? { backgroundImage: `url(${hero})` } : {}}>
        ...
        <div className="hero-body">
          {business_data.city && <p className="eyebrow">{business_data.city}, {business_data.state}</p>}
          <h1>{copy.headline}</h1>
          <p className="lead">{copy.subheadline}</p>
          <a href="#contact" className="btn">{copy.cta_text || 'Book a Table'}</a>
        </div>
      </section>
      ...
      <section className="info" id="contact">
        <div className="wrap info-grid">
          ...
          {business_data.phone && (
            <div>
              <span className="section-tag">Call Us</span>
