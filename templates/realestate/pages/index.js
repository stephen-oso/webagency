import Head from 'next/head';

export default function Page({ data }) {
  const { copy = {}, photos = [], business_data = {} } = data || {};
  const hero = photos[0] || null;
  const gallery = photos.slice(1, 7);
  const hours = business_data.hours?.weekday_text || [];

  return (
    <>
      <Head>
        <title>{business_data.name || 'Real Estate'}</title>
        <meta name="description" content={copy.meta_description || ''} />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link rel="preconnect" href="https://fonts.googleapis.com" />
        <link rel="preconnect" href="https://fonts.gstatic.com" crossOrigin="anonymous" />
        <link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,wght@0,300;0,600;1,300&family=Raleway:wght@300;400;600&display=swap" rel="stylesheet" />
      </Head>

      <nav className="nav">
        <div className="nav-inner">
          <span className="nav-brand">{business_data.name}</span>
          <a href="#contact" className="nav-cta">{copy.cta_text || 'Contact Agent'}</a>
        </div>
      </nav>

      <section className="hero" style={hero ? { backgroundImage: `url(${hero})` } : {}}>
        <div className="hero-veil" />
        <div className="hero-body">
          <p className="eyebrow">{business_data.city}, {business_data.state}</p>
          <h1>{copy.headline}</h1>
          <p className="hero-sub">{copy.subheadline}</p>
          <div className="hero-actions">
            <a href="#contact" className="btn-primary">{copy.cta_text || 'Start Your Search'}</a>
            {business_data.phone && (
              <a href={`tel:${business_data.phone}`} className="btn-outline">{business_data.phone}</a>
            )}
          </div>
        </div>
      </section>

      <section className="about">
        <div className="wrap about-inner">
          <div className="about-text">
            <span className="section-tag">About</span>
            <p className="about-copy">{copy.about}</p>
          </div>
          <div className="about-card">
            {hero && <div className="agent-img" style={{ backgroundImage: `url(${hero})` }} />}
            <div className="agent-info">
              <p className="agent-name">{business_data.name}</p>
              <p className="agent-loc">{business_data.city}, {business_data.state}</p>
              {business_data.rating && (
                <p className="agent-rating">★ {business_data.rating} · {business_data.review_count?.toLocaleString()} reviews</p>
              )}
            </div>
          </div>
        </div>
      </section>

      {copy.services?.length > 0 && (
        <section className="services">
          <div className="wrap">
            <span className="section-tag">How I Help</span>
            <h2>Services</h2>
            <div className="services-grid">
              {copy.services.map((s, i) => (
                <div key={i} className="service-card">
                  <div className="card-accent" />
                  <span>{s}</span>
                </div>
              ))}
            </div>
          </div>
        </section>
      )}

      {gallery.length > 0 && (
        <section className="gallery">
          {gallery.map((src, i) => (
            <div key={i} className="gallery-cell">
              <img src={src} alt="" loading="lazy" />
            </div>
          ))}
        </section>
      )}

      <section className="info" id="contact">
        <div className="wrap">
          <h2>Work With Me</h2>
          <div className="info-grid">
            <div>
              <span className="section-tag">Market</span>
              <p>{business_data.city}, {business_data.state}</p>
            </div>
            {business_data.phone && (
              <div>
                <span className="section-tag">Phone</span>
                <a href={`tel:${business_data.phone}`}>{business_data.phone}</a>
              </div>
            )}
            {business_data.email && (
              <div>
                <span className="section-tag">Email</span>
                <a href={`mailto:${business_data.email}`}>{business_data.email}</a>
              </div>
            )}
            {hours.length > 0 && (
              <div>
                <span className="section-tag">Availability</span>
                {hours.map((h, i) => <p key={i} className="hour-row">{h}</p>)}
              </div>
            )}
          </div>
          <a href={business_data.phone ? `tel:${business_data.phone}` : `mailto:${business_data.email}`} className="contact-cta">
            {copy.cta_text || 'Get in Touch'}
          </a>
        </div>
      </section>

      <footer>
        <p>© {new Date().getFullYear()} {business_data.name} · {business_data.city}, {business_data.state}</p>
      </footer>
    </>
  );
}

export async function getStaticProps() {
  const data = require('../site_data.json');
  return { props: { data } };
}
